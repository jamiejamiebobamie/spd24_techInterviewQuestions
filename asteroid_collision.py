...
def collision(asteroid_array):
    ...
    for i, asteroid in enumerate(asteroid_array):
        ...
        # else if it's moving left
            # check if there are any asteroids in the collider array
            # (moving right) and if there are, pop the the closest one.
        else:
            if len(colliders):
                collider = colliders.pop()
                collider_index = collider_index_array.pop()
        # if we have a collision (if collider is not None)
            # check the current asteroid against the collider
        while collider and asteroid:
            ...
            if abs(collider) == abs(asteroid):
                ...
                collider = None
                asteroid = None
            # if the right side asteroid (the collider) is larger.
                # set the (left side) asteroid to None
